module.exports = {
        ...
        create: (req, res) => {
        const dbInstance = req.app.get('db');
        const { name, info, address, city, c_state, zipcode, image, loan_amount, mortgage, des_rent, rec_rent } = req.body
        const { user_id } = req.session.user

        dbInstance.add_house([name, info, address, city, c_state, zipcode, image, loan_amount, mortgage, des_rent, rec_rent, user_id])
        .then(houses => {
            res.status(200).send(houses)
        }).catch( err => {
            console.log(err)
